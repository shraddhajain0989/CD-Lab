...
int main()
{
char inp[80],x,p,dl[80],y,bl='a'; int i=0,j,k,l,n,m,len;

printf("Enter the input: "); scanf("%s",inp);

len=strlen(inp); inp[len]='$';
inp[len+1]='\0'; push(stack,&top,bl);
printf("\nSTACK\t\tINPUT\n"); printt(stack,&top,inp,i);

do
{
x=inp[i]; p=stacktop(stack); isproduct(x,p);
...
{
j=isstate(temp[1]); strcpy(temp,rl[j-2].right);
dl[0]=rl[j-2].left;
dl[1]='\0';
...
printt(stack,&top,inp,i);
}while(inp[i]!='\0'); if(strcmp(temp,"acc")==0)
printf("\nAccepted"); else
printf("\nNot Accepted");
...
{
if(*sp==100) printf("Stack is full");
else
{
*sp=*sp+1; s[*sp]=item;
}
...
{
int i; for(i=0;i<6;i++)
if(x==ter[i]) return i+1; return 0;
}

int isnter(char x)
{
int i; for(i=0;i<3;i++)
if(x==nter[i]) return i+1; return 0;
}

int isstate(char p)
{
int i; for(i=0;i<12;i++)
if(p==states[i]) return i+1; return 0;
}

void error()
{
printf("\nError in input"); exit(0);
}
